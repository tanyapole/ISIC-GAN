...
if [ -z "${VALIDATION}" ]
then
  echo "required VALIDATION argument"
  exit 1
fi
echo "$VALIDATION"
echo ""

if [ "$VALIDATION" != "validation_skin_lesion" ] && [ "$VALIDATION" != "validation" ]
then
  echo "must be validation_skin_lesion or validation"
  exit 1
fi
...
validate_csv="/nfs/home/nduginets/master-diploma/splits_boxed/$VALIDATION.csv"

if [ "$VALIDATION" == "validation_skin_lesion" ]
then
  GENERATED_PATH="generated_bussio"
  EXTENSION="bussio_"
fi
...
train_csv="/nfs/home/nduginets/master-diploma/splits/$GENERATED_PATH/train_${EXTENDED}_${split}.csv"
CUDA_VISIBLE_DEVICES=$DEVICES python3 train.py \
                                ...
                                --result_dir ${result_dir} --experiment_name "${EXTENSION}boxed_pix_2pix_classifier_${EXTENDED}_${split}"
done
