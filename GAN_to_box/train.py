...
def train_epoch(device,
    ...
    for data in tqdm_loader:
        ...
        bounding_boxes_data = bounding_boxes_data.to(device)
        ...
        with torch.set_grad_enabled(phase == 'train'):
            ...
            ######################
            # todo
            # todo maybe add sigmoid activation ? noise will be in
            # todo

            true_label = torch.full((batch_size,), 1.0, dtype=torch.float, device=device)

            true_D_output = discriminator(bounding_boxes_data).view(-1)  # already with sigmoid todo removed
            ...
            fake_D_output = discriminator(fake_G_output).view(-1)  # todo removed
            ...
            output = discriminator(generated_fake_data).view(-1)  # todo removed
        ...
        tqdm_loader.set_postfix(loss=losses["sum"].avg, acc=accuracies["sum"].avg, epoch=epoch_number)
...
def main(train_csv, val_csv, epochs: int, batch_size: int,
         num_workers, experiment_path):
